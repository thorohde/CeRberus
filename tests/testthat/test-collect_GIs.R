...
test_that("collect_GIs extracts fixed-pair coefficients, p-values, and adjusted FDR", {
  ...
  expect_equal(unname(result@geneGIs[, "FDR"]), stats::p.adjust(c(0.01, 0.20, 0.03), method = "BH"))
})
...
test_that("collect_GIs respects requested FDR method for fixed-pair screens", {
  ...
  expect_equal(unname(result@geneGIs[, "FDR"]), stats::p.adjust(c(0.01, 0.20, 0.03), method = "bonferroni"))
})
...
test_that("collect_GIs rejects unknown FDR methods", {
  ...
  screen <- make_collect_screen(
    class = "FixedPairScreen",
    guideGIs = make_collect_guideGIs(guide_data, space = "gene_pair"),
    limma_models = make_collect_model(rownames(guide_data), c(1, 2, 3), c(0.1, 0.2, 0.3))
  )
...
test_that("collect_GIs builds a query-by-library-by-variable array for multiplex screens", {
  ...
  screen <- make_collect_screen(
    ...
    guideGIs = make_collect_guideGIs(guide_data, space = c("query_gene", "library_gene")),
    limma_models = list(
      A = make_collect_model(c("A", "B", "C"), c(0.1, 0.2, 0.3), c(0.01, 0.02, 0.03)),
      B = make_collect_model(c("A", "B", "C"), c(-0.1, -0.2, -0.3), c(0.04, 0.05, 0.06)),
      C = make_collect_model(c("A", "B", "C"), c(0.7, 0.8, 0.9), c(0.07, 0.08, 0.09))
    ),
    screen_attr = list(
      query_genes = c("A", "B", "C"),
  ...
  expect_equal(unname(result@geneGIs["A", , "FDR"]), stats::p.adjust(c(0.01, 0.02, 0.03), method = "BH"))
})
...
test_that("collect_GIs creates symmetrized output for position-agnostic multiplex screens", {
  ...
  screen <- make_collect_screen(
    ...
    guideGIs = make_collect_guideGIs(guide_data, space = c("query_gene", "library_gene")),
    limma_models = list(
      A = make_collect_model(c("A", "B", "C"), c(0.0, 0.2, 0.4), c(0.50, 0.01, 0.02)),
      B = make_collect_model(c("A", "B", "C"), c(0.3, 0.0, 0.5), c(0.03, 0.50, 0.04)),
      C = make_collect_model(c("A", "B", "C"), c(0.6, 0.7, 0.0), c(0.05, 0.06, 0.50))
    ),
    screen_attr = list(
      query_genes = c("A", "B", "C"),
      library_genes = c("A", "B", "C"),
  ...
  expect_named(result@symmGeneGIs, c("gene_pair", "query_gene", "library_gene", "GI", "GI_z", "pval", "FDR"))
